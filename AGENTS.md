...
- `src/rendered.ts` rendered-page mode: fetched HTML → markdown subset → Drift. Sits beside markdown mode (`routes`); config `site` + `pages` (+ `root`). Needs semantic HTML (`<pre>`, headings). Build-time only, never imported by the overlay.
...
- The seam: a rule hit is the ledger (code checked it, certain, "Proved"); a Jev score is a sample (a probability, "Likely"). The sample never writes on the ledger: Jev is not asked about rule hits or gaps, and exact checks (arity, option keys, literal types, parameter tables, imports, required arguments) are Drift rules, never Jev questions. Jev keeps what code cannot do: stale, what prose says, written-out types, members named in prose, setup.
